subroutine cadena_v2 (comm)
    ...
    if (myid == 0) then
      ...
      do j = 0,longitud
          ...
          enddo
          resultados(j+1) = acum/dble(veces)
      enddo
      do j = 1,longitud+1
       print *, j, resultados(j)/numprocs
      enddo
      !print 10,(j,resultados(j)/numprocs,j=1,longitud+1)
      !10 FORMAT (1X, I3, F12.7)
      !! final
