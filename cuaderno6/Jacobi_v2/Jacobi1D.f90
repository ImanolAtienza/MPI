...
!
      ...
      integer maxn
      double precision, allocatable :: a(:,:), b(:,:), fheat(:,:), w(:,:)
      integer nx, ny
      ...
      double precision gconf(4) , conv(3), chip(6)
      ...
      if (myid .eq. 0) then
           call readParam(gconf, conv, chip)
      ...
      maxStep = conv(1)/ 8
      convTot = conv(3)
      call MPI_BCAST(chip,6,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)

      kdim(1) = .false.
      nnproc(1)=numprocs
      call MPI_CART_CREATE( MPI_COMM_WORLD, 1, nnproc, kdim, &
                            .true., comm1d, ierr )
   ! call MPI_CART_CREATE( MPI_COMM_WORLD, 1, numprocs, kdim, &
     !                       .true., comm1d, ierr )
      ...
      call onedinit( a, b, fheat, nx, ny, s, e, tmp_ambiente,chip, factor_escala)
