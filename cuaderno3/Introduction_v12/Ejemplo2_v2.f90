subroutine ejemplo2_v2 (comm, n, stride)
use mpi
implicit none
...
allocate (req(nprocs))
!stride = CEILING (dble(n)/dble(nprocs))
...
If (rank .eq. master) then ! master
 ...
 do i = 1, vecessendreceive
    ...
    call MPI_ISend (vec(current), cuantos, MPI_INTEGER,&
                    aquienenvio, 1, comm,&
                    req(aquienenvio+1), ierr)
    ...
    if (j ==  nprocs-1) then ! he enviado a todos los slaves
     do k = 0, nprocs-2
         ...
         !! MPI_Request_free (req,ierr)
         call MPI_Request_free(req(k+1),ierr)
...
ELSE  ! slaves
    ...
    do
      ...
      ENDDO
      total = total + sum
      call MPI_ISend (sum, 1, MPI_INTEGER,&
                  master, rank, comm, req(rank+1),ierr)
   end do
ENDIF
